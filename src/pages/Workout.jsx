  ...
  return (
    ...
    <div className="max-w-lg mx-auto p-5 space-y-5">
      <div className="pt-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">{GOAL_LABELS[profile.goal]} • {profile.sessionLength} session</p>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
        {plan.map((d, i) => (
          <button
            ...
            className={`flex-shrink-0 px-4 py-3 min-h-[44px] rounded-xl text-sm font-medium transition-all ${selectedDay === i ? 'bg-emerald-500 text-white' : 'bg-card border border-border text-muted-foreground'}`}
          >
            Day {d.day}
          </button>
        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {dayData?.muscles?.map((m) => (
          <span key={m} className="px-3 py-1 bg-muted rounded-lg text-xs text-foreground">{m}</span>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {dayData?.exercises?.map((ex, i) => (
          <div key={i} className={`bg-card rounded-2xl overflow-hidden border transition-all ${completed[ex.name] ? 'border-emerald-500/30 opacity-60' : 'border-border'}`}>
            <button
            ...
            >
              ...
              >
                <span className={`w-6 h-6 rounded-full border-2 block transition-all ${completed[ex.name] ? 'bg-emerald-500 border-emerald-500' : 'border-muted-foreground'}`} />
              </span>
              ...
              {expandedEx === i ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}
            </button>

            {expandedEx === i && (
              <div id={`ex-detail-${i}`} role="region" aria-label={`${ex.name} details`} className="px-4 pb-4 pt-0">
                <p className="text-sm text-muted-foreground border-t border-border pt-3">{ex.desc}</p>
              </div>
            )}
          </div>
        ))}
      ...
      {completedCount === total && total > 0 && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-5 text-center">
          ...
          <div className="text-sm text-muted-foreground mt-1">Great job. Rest & recover.</div>
        </div>
      )}
